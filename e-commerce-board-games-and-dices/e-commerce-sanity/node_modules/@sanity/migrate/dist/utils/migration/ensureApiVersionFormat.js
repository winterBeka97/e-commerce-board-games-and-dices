var VERSION_PATTERN = /^v\d+-\d+-\d+$|^vX$/ // Matches version strings like vYYYY-MM-DD or vX
;
/**
 ...
 */ export function ensureApiVersionFormat(version) {
    var normalizedVersion = version.startsWith('v') ? version : "v".concat(version);
    // Check if the version matches the expected pattern
    if (!VERSION_PATTERN.test(normalizedVersion)) {
        throw new Error("Invalid API version format: ".concat(normalizedVersion, ". Expected format: vYYYY-MM-DD or vX"));
    }
